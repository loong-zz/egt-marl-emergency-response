...
def run_system_test(args):
    ...
    if args.run_tests:
        ...
        for mode in test_modes:
            if mode == 'quick' or mode == args.test_mode:
                cmd = f'python tests/run_all_tests.py --mode {mode}'
    ...
    if args.train_egt_marl:
        ...
        output_dir = f"experiment_results/system_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        cmd = f'python experiments/train_egt_marl.py '
    ...
    if args.evaluate_baselines:
        ...
        output_dir = f"evaluation_results/system_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        cmd = f'python experiments/evaluate_baselines.py '
        ...
        if args.quick_mode:
            cmd += f'--num_runs 1 '
            # 快速测试模式下，只评估一个场景、一个严重程度和最少算法
            cmd += f'--scenarios earthquake '
            cmd += f'--severities low '
            cmd += f'--algorithms fcfs '
    ...
    if args.run_ablation:
        ...
        output_dir = f"ablation_results/system_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        cmd = f'python experiments/ablation_study.py '
        cmd += f'--config configs/ablation.yaml '
        cmd += f'--output_dir {output_dir} '
        
        if args.quick_mode:
            cmd += f'--num_runs 1 '
            cmd += f'--num_episodes 5 '
            cmd += f'--components egt_layer '
    ...
    if args.test_robustness:
        ...
        output_dir = f"robustness_results/system_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        cmd = f'python experiments/robustness_test.py '
    ...
    for base_dir in base_dirs:
        if os.path.exists(base_dir):
            # 找到最新的system_test目录
            dirs = [d for d in os.listdir(base_dir) if d.startswith('system_test_')]
            if dirs:
                latest = max(dirs)
                print(f"  {base_dir}/{latest}/")
    
    if all_success:
        print("\n[SUCCESS] 系统测试所有任务完成成功！")
        print("\n下一步建议:")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='运行系统集成与测试')
    ...
    parser.add_argument('--quick-mode', action='store_true', 
                       help='快速测试模式，减少episode数量以加快测试速度')
    ...
    # 快速测试模式处理
    if args.quick_mode:
        print("启用快速测试模式，减少episode数量以加快测试速度")
        if not args.num_episodes:
            args.num_episodes = 10  # 设置为10个episode以加快测试
        # 在快速模式下，使用quick测试模式
        if args.test_mode == 'comprehensive':
            args.test_mode = 'quick'
    
    # 运行系统集成测试
    success = run_system_test(args)
